...
(function(b) {
	var a = {
		...
		ean : {
			...
			getDigit : function(j, h) {
				...
				if (h == "ean8") {
					...
					for (var g = 1; g < 7; g++) {
						d += this.encoding[a.intval(j.charAt(g))][a.intval(f
								.charAt(g - 1))]
					}
					d += "01010";
					for (var g = 7; g < 13; g++) {
		...
		code93 : {
			...
			getDigit : function(e, j) {
				var n = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZ-. $/+%____*", l, o = "";
				if (e.indexOf("*") >= 0) {
					return ("")
				}
				e = e.toUpperCase();
		...
		code128 : {
			...
			getDigit : function(e) {
				...
				while (k < e.length) {
					if (!g) {
						h = 0;
						while ((k + h < e.length) && (e.charAt(k + h) >= "0")
								&& (e.charAt(k + h) <= "9")) {
							h++
						}
		...
		digitToBmpRenderer : function(t, v, o, g, n, z) {
			...
			var f = o[0].length;
			var u = 0;
			var r = this.isHexColor(v.bgColor) ? this.lec.cHexColor(v.bgColor)
					: this.lec.cRgb(255, 255, 255);
			var q = this.isHexColor(v.color) ? this.lec.cHexColor(v.color)
					: this.lec.cRgb(0, 0, 0);
			var d = "";
			var c = "";
			for (u = 0; u < n; u++) {
			...
			B.setAttribute("data", "data:image/bmp;base64,"
					+ this.base64Encode(A));
		...
		digitToCssRenderer : function(q, c, m, e, p, g) {
			...
			if (c.showHRI) {
				j += '<div style="clear:both; width: 100%; background-color: '
						+ c.bgColor + "; color: " + c.color
						+ "; text-align: center; font-size: " + c.fontSize
						+ "px; margin-top: " + c.marginHRI + 'px;">' + e
						+ "</div>"
			}
		...
		digitToSvgRenderer : function(q, s, m, f, k, t) {
			...
			var n = '<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="'
					+ p + '" height="' + l + '">';
			n += '<rect width="' + p + '" height="' + l
			...
			for (var h = 0; h < d; h++) {
				...
				if ((r > 0) && (o == "1")) {
					n += c.replace("&W", r * k).replace("&X", (e - r) * k)
							.replace("&Y", h * t)
				}
			}
			if (s.showHRI) {
		...
		digitToCanvasRenderer : function(s, e, o, g, m, f, q, k) {
			...
			if (e.showHRI) {
				...
				r.fillText(g, m + Math.floor((d * q - h.width) / 2), f + t * k
						+ e.fontSize + e.marginHRI)
			}
		},
	...
	b.fn.extend({
		barcode : function(h, m, f) {
			var n = "", g = "", d = "", k = true, o = false, j = false;
			if (typeof (h) == "string") {
				d = h
			} else {
				if (typeof (h) == "object") {
